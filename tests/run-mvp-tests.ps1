...
Assert-True ($indexHtml.Contains("id=""jobs""")) "static GUI exposes job history panel"
...
Assert-True ($appJs.Contains("/events")) "static GUI calls Agent job events endpoint"
Assert-True ($appJs.Contains("/cancel")) "static GUI calls Agent job cancel endpoint"
Assert-True ($appJs.Contains("/retry")) "static GUI calls Agent job retry endpoint"
Assert-True ($appJs.Contains("toggleJobLog")) "static GUI toggles job log preview"
Assert-True ($appJs.Contains("EventSource")) "static GUI can stream job progress"
Assert-True ($appJs.Contains("startJobStream")) "static GUI starts job event streams"
Assert-True ($appJs.Contains("cancelJob")) "static GUI can request job cancellation"
Assert-True ($appJs.Contains("retryJob")) "static GUI can retry failed jobs"
Assert-True ($appJs.Contains("refreshExpandedJobLogs")) "static GUI refreshes expanded job logs"
Assert-True ($appJs.Contains("completedAt")) "static GUI renders job completion time"
Assert-True ($appJs.Contains("exitCode")) "static GUI renders job exit code"
Assert-True ($appJs.Contains("executeViaAgent")) "static GUI can execute through local Agent"
Assert-True ($appJs.Contains("riskForCommand")) "static GUI assesses command risk"
Assert-True ($appJs.Contains("riskRemoteScript")) "static GUI describes remote script risk"
Assert-True ($appJs.Contains("confirmHighRisk")) "static GUI sends high risk confirmation"
Assert-True ($appJs.Contains("highRiskConfirm")) "static GUI confirms high risk execution"
Assert-True ($appJs.Contains("confirmUntrusted")) "static GUI sends untrusted recipe confirmation"
Assert-True ($appJs.Contains("trustForRecipe")) "static GUI renders recipe trust"
Assert-True ($stylesCss.Contains(".agent-status")) "static GUI styles local Agent status"
Assert-True ($stylesCss.Contains(".risk-high")) "static GUI styles high risk actions"
Assert-True ($stylesCss.Contains(".trust-warn")) "static GUI styles untrusted actions"
Assert-True ($stylesCss.Contains(".job")) "static GUI styles job history"
Assert-True ($stylesCss.Contains(".job-status")) "static GUI styles job status"
Assert-True ($stylesCss.Contains(".cancel-job")) "static GUI styles job cancellation"
Assert-True ($stylesCss.Contains(".retry-job")) "static GUI styles job retry"
